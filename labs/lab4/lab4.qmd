...
For a note at frequency `f`, the GPIO pin must be toggled twice per period (once high, once low), so the timer must have double the intended frequency `2f`. Then, we can write an expression for the Auto-Reload Register `ARR`:
...
$$
f_{\text{max}} = \frac{f_{\text{timer}}}{2 \times (\text{ARR}_{\text{min}} + 1)} = \frac{1{,}000{,}000}{2 \times 2} = 250{,}000\,\text{Hz}
$$


Note that because TIM2 is a 32-bit timer, ARR_max = $2^{32} - 1$ = 4,294,967,295. Both values comfortably cover the required 220–1000 Hz range.
...
`Claude Sonnet` also provided the correct register configuration, and how to enable the clocks with RCC, and even configure the GPIO pin! Overall the LLM returned mostly the correct information, however it was a litle too much information than I asked for.